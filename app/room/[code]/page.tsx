...
const MODE_CONFIG: Record<QuestionMode, { label: string }> = {
  both:            { label: 'Integration & Differentiation' },
  integration:     { label: 'Integration only' },
  differentiation: { label: 'Differentiation only' },
}
      ...
      {entries.map(([id, p]) => (
        <div key={id} className={`p-2 rounded-lg ${id === userId ? 'bg-gold/10' : 'bg-white/5'}`}>
          ...
          <div className="w-full h-1.5 bg-white/10 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-300 ${p.done ? 'bg-emerald-400' : id === userId ? 'bg-gold' : 'bg-cream/40'}`}
...
export default function RoomCodePage() {
  ...
  useEffect(() => {
    ...
    const subscribe = async () => {
      ...
      const unsub = onSnapshot(ref, async snap => {
        ...
        // Join room if not already a player
        ...
        // Detect lobby → playing transition to init local game state
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        if (isLast) {
          ...
          await updateDoc(ref, {
            [`players.${userId}.progress`]: nextIndex,
          })
          setResults(newResults)
          setQIndex(nextIndex)
          setAnswer('')
  ...
  const resetRoom = async () => {
    ...
    await updateDoc(ref, { status: 'lobby', questions: [], startedAt: null, players })
    setMyDone(false)
    setResults([])
    setQIndex(0)
    setAnswer('')
        ...
        <div className="max-w-lg w-full space-y-6 animate-fade-in">
          <div className="text-center">
            ...
            <button
              type="button"
              onClick={copyCode}
              className="font-mono text-2xl text-gold tracking-[0.2em] hover:opacity-70 transition-opacity cursor-pointer"
              title="Click to copy"
            >
              {code}
            </button>
            {copied && <p className="text-cream/40 text-xs font-mono mt-1">Copied!</p>}
            {!copied && <p className="text-cream/30 text-xs font-mono mt-1">Click code to copy · Share with friends</p>}
          </div>
          ...
          <div className="card space-y-4">
            ...
            <div>
              <div className="text-cream/60 text-xs font-mono mb-2">Difficulty</div>
                ...
                {(['easy', 'medium', 'hard'] as Difficulty[]).map(d => {
                  ...
                  return (
                    <button
                      ...
                      className={`py-2 rounded-lg border text-xs font-mono transition-all duration-150 ${
            ...
            <div>
              <div className="text-cream/60 text-xs font-mono mb-2">Question Type</div>
              <div className="grid grid-cols-3 gap-2">
                {(['both', 'integration', 'differentiation'] as QuestionMode[]).map(m => {
                  const sel = mode === m
                  const label = m === 'both' ? 'Both' : m === 'integration' ? '∫ Integration' : 'd/dx Diff.'
                  return (
                    <button
                      key={m}
                      type="button"
            ...
            {!isHost && (
              <p className="text-cream/30 text-xs font-mono">Only the host can change settings</p>
            )}
          </div>
          ...
          {isHost ? (
            <button
              ...
              onClick={startGame}
              disabled={starting}
              className="w-full py-4 text-xl rounded-xl font-semibold cursor-pointer transition-all duration-200 hover:opacity-90 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          <div className="text-center">
            <p className="text-cream/30 text-xs font-mono">
              Mode: {MODE_CONFIG[mode].label} · 10 questions
            </p>
          </div>
        </div>
      </main>
  ...
  if (room.status === 'playing') {
    ...
    // My turn is done — waiting for others
    if (myDone) {
      return (
        <main className="min-h-screen flex flex-col">
          <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
            <div className="font-mono text-sm text-cream/40">Room {code}</div>
    ...
    return (
      <main className="min-h-screen flex flex-col">
        ...
        <div className="flex flex-1 min-h-0">
          ...
          {/* Right spacer — desktop only, keeps question centered */}
  ...
  // ─── Finished ─────────────────────────────────────────────────────────────
  // Show final scores. Also show if all players are done even if status isn't 'finished'.
  const showFinished = room.status === 'finished' || allDone
      ...
      <main className="min-h-screen px-4 py-10">
        <div className="max-w-xl mx-auto space-y-6 animate-fade-in">
          <div className="text-center">
            ...
            <p className="text-cream/40 font-mono text-sm mt-1">Room {code} · {MODE_CONFIG[room.settings.mode].label}</p>
          </div>
          ...
          {isHost && (
            <button
              type="button"
              onClick={resetRoom}
              className="w-full py-3 rounded-xl font-semibold cursor-pointer transition-all duration-200 hover:opacity-90 active:scale-95"
              style={{ backgroundColor: 'var(--gold)', color: 'var(--navy)', fontFamily: 'Playfair Display, serif' }}
            >
              Play Again
            </button>
          ...
          <div className="flex gap-4">
            <Link href="/room" className="btn-outline flex-1 text-center py-3">
              Leave Room
            </Link>
